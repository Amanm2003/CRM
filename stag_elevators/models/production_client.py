...
class ProdClient(models.Model):
    ...
    lead_model = fields.Text(
        string="Model",
        compute='_compute_fields_from_lead',
        inverse='_inverse_fields_to_lead',
        store=False
    )
    floor = fields.Text(
        string="Floor",
    ...
    shaft = fields.Selection(
        [('Metal', 'Metal'), ('Platform_lift', 'Platform lift'), ('Civil', 'Civil')],
        string='Shaft',
        compute='_compute_fields_from_lead',
        inverse='_inverse_fields_to_lead',
        store=False
    ...
    @api.depends('lead_id')
    def _compute_fields_from_lead(self):
        for record in self:
            if record.lead_id:
                crm_client = self.env['crm.client'].search([('lead_id', '=', record.lead_id.id)], limit=1)
                if crm_client:
                    record.lead_model = crm_client.lead_model
                    record.floor = crm_client.floor
                    record.shaft = crm_client.shaft
                else:
                    record.lead_model = False
                    record.floor = False
                    record.shaft = False
            else:
                record.lead_model = False
    ...
    def _inverse_fields_to_lead(self):
        for record in self:
            if record.lead_id:
                crm_client = self.env['crm.client'].search([('lead_id', '=', record.lead_id.id)], limit=1)
                if crm_client:
                    crm_client.write({
                        'lead_model': record.lead_model,
                        'floor': record.floor,
                        'shaft': record.shaft,
                    })
