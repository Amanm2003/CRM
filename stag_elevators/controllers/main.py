...
# =============== working model ===============
    ...
    def import_leads_submit(self, **post):
        ...
        try:
            if filename.endswith('.csv'):
                ...
                for idx, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
                    ...
                    if not any(row) or all(str(cell).strip() == "" for cell in row if cell is not None):
                        continue
        ...
        # ✅ If errors exist, show them with imported summary
        if errors:
            return request.render('stag_elevators.import_error_template', {
                'error': f"{imported_count} rows inserted; below rows skipped due to errors:",
                'errors': errors
            })

        # ✅ If all good, redirect
        # return request.redirect('/opportunities')
        return request.render('stag_elevators.import_success_template', {
